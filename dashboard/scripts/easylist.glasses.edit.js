...
function LoadGlassEditInfo()
{
	var Make = getURLParameter('Make');
	var Model = getURLParameter('Model');
	var Year = getURLParameter('Year');
	var Style = getURLParameter('Style');
	var Transmission = getURLParameter('Transmission');
	var Variant = getURLParameter('Variant');
	
    $.getJSON(GlassAPIURL + 'EditGlassInfoHTML&Make=' + Make + '&Model=' + Model + '&Year=' + Year + '&Style=' + Style + '&Transmission=' + Transmission + '&Variant=' + Variant, function (data) {
        $("#AutomotiveListingEdit").html(jQuery.parseJSON(data.Result));
    });
}

function LoadMotorGlassEditInfo()
{
	var Make = getURLParameter('Make');
	var Model = getURLParameter('Model');
	var Year = getURLParameter('Year');
    ...
    $.getJSON(GlassAPIURL + 'EditGlassMotorInfoHTML&Make=' + Make + '&Model=' + Model + '&Year=' + Year + '&Variant=' + Variant, function (data) {
        $("#AutomotiveListingEdit").html(jQuery.parseJSON(data.Result));
    });
}
...
function MakeModelYearStyleChange(element, type, refreshControlName) {
    ...
    */
    ...
    $.getJSON(GlassAPIURL + 'Info&Type=' + type + SearchParam, function (data) {
		...
		DefaultItemsList.push('<option value="">Select an option</option>');
        items.push('<option value="">Select an option</option>');
        $.each(jQuery.parseJSON(data.Result), function (key, value) {
            items.push('<option value="' + value + '">' + value + '</option>');
        });
		...
		var refreshControlNameList = refreshControlName.split(',');
		for (var i = 0; i < refreshControlNameList.length; i++) {
			if (i == 0)
			{
				$(refreshControlNameList[i]).html(items.join(''));
			}else
			{
				$(refreshControlNameList[i]).html(DefaultItemsList.join(''));
			}
		}
...
function VariantEditChange(element) {
     ...
     $.getJSON(GlassAPIURL + 'MatchingVehicleFeatures&GlassCode=' + GlassCode
         ...
         $.each(jQuery.parseJSON(data.Result), function (key, value) {
			 ...
			 $('#easylist-standard-features').val(vehicleFeature.StandardFeaturesName);
			 $('#easylist-optional-features').val(vehicleFeature.OptionalFeaturesName);
...
function MotorMakeModelYearChange(element, type, refreshControlName) {
 ...
 function MotorYearChange(element) {
    ...
    $.getJSON(GlassAPIURL + 'MatchingMotor&make=' + Make + '&model=' + Model + '&year=' + Year, function (data) {
        var items = [];
        items.push('<option value="Select">Select an option</option>');
        $.each(jQuery.parseJSON(data.Result), function (key, value) {
...
function MotorVariantEditChange(element)
{
	//if ($(element).val() == 'Select') return;
    if ($(element).val() == "Select") {
        $('#group-motor-features').hide();
...
function GetTransType(TransDesc)
{
	var TransType = "";
	try
	{
		if (TransDesc.toUpperCase().indexOf("AUTO") > 0)
		{
			TransType = "Automatic";
		}
		else
...
function GetBodyStyle(BodyDesc)
{
	var BodyStyle = "";
	try
	{
		var BodyDescUpperCase = BodyDesc.toUpperCase();
		if (BodyDescUpperCase.indexOf("4x4") > 0)
		{
			BodyStyle = "4x4";
		}
		else if (BodyDescUpperCase.indexOf("CONVERTIBLE") > 0)
		{
			BodyStyle = "Convertible";
		}
		else if (BodyDescUpperCase.indexOf("COUPE") > 0)
		{
			BodyStyle = "Coupe";
		}
		else if (BodyDescUpperCase.indexOf("HATCHBACK") > 0)
		{
			BodyStyle = "Hatchback";
		}
		else if (BodyDescUpperCase.indexOf("PEOPLE MOVER") > 0)
		{
			BodyStyle = "People Mover";
		}
		else if (BodyDescUpperCase.indexOf("ROADSTER") > 0)
		{
			BodyStyle = "Roadster";
		}
		else if (BodyDescUpperCase.indexOf("SUV") > 0)
		{
			BodyStyle = "SUV";
		}
		else if (BodyDescUpperCase.indexOf("SEDAN") > 0)
		{
			BodyStyle = "Sedan";
		}
		else if (BodyDescUpperCase.indexOf("SPORTS") > 0)
		{
			BodyStyle = "Sports";
		}
		else if (BodyDescUpperCase.indexOf("TRUCK") > 0)
		{
			BodyStyle = "Truck/Bus";
		}
		else if (BodyDescUpperCase.indexOf("BUS") > 0)
		{
			BodyStyle = "Truck/Bus";
		}
		else if (BodyDescUpperCase.indexOf("UTE") > 0)
		{
			BodyStyle = "Ute";
		}
		else if (BodyDescUpperCase.indexOf("VAN") > 0)
		{
			BodyStyle = "Van";
		}
		else if (BodyDescUpperCase.indexOf("WAGON") > 0)
		{
			BodyStyle = "Wagon";
		}
	}
