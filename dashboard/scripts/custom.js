...
$(document).ready(function(){
	...
	// Added 8/4/2013 - Check if footable tab is changed, if is trigger resize event.
	if($('.footable').length > 0) {
		$('.widget-box [data-toggle="tab"]').on('click',function(){
			// Fake loading
			window.setTimeout(function(){ $('.active #loading').show(); },10);
			window.setTimeout(function(){ $('.active #loading').fadeOut(); }, 1000);
			// Trigger resize func
			window.setTimeout(function(){
				$('.tab-pane.active .footable').trigger('resize');
			},20);
		});
	}
...
function template_functions(){
	...
	$('.cleditor').cleditor(
		{width:"100%",
		 controls:     // controls to add to the toolbar
                        "bold italic underline strikethrough subscript superscript | removeformat | bullets numbering | undo redo | " +
	...
	/* ---------- Custom Slider ---------- */
		...
		var gmmin = $('#gm-price-min');
		var gmmax = $('#gm-price-max');
		var gmval = [ 0, 500 ];
		if(gmmin.val() !== "") {
			gmval = [gmmin, gmmax];
		}
		$( ".gmsliderRange" ).slider({
			range: true,
			min: 0,
			max: 1000,
			values: gmval,
			slide: function( event, ui ) {
				$( ".gmsliderRangeLabel" ).html( "$" + ui.values[ 0 ] + " - $" + ui.values[ 1 ] );
			}
		});
