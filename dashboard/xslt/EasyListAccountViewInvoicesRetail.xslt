...
<xsl:template match="/">
	<xsl:variable name="IsAuthorized" select="AccScripts:IsAuthorized('RetailUser')" /> 
	<xsl:choose>
...
<!-- start writing XSLT -->
		...
		<xsl:otherwise>
			...
			<xsl:if test="count($InvoiceList/ArrayOfELInvoice/ELInvoice) = 0">
				 <div id="easylist-no-results" class="alert">
					You don't have any invoice to display!
				 </div>
			</xsl:if>
		</xsl:otherwise>
	</xsl:choose>
 ...
 <xsl:template name="Invoice-item" match="ElInv">
    ...
    <tr>
		...
		<td>
			<a class="EditListing" href="/account/retail-invoice?InvNo={$ElInv/InvoiceNumber}">
